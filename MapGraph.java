...
public class MapGraph {
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		// Setup - check validity of inputs
				...
				for(GeographicPoint location: pointNodeMap.keySet()) {
					pointNodeMap.get(location).setCost(Double.POSITIVE_INFINITY);
				}
				toExplore.add((new MapNode(startNode, 0.0)));
				MapNode next = null;
				...
				while (!toExplore.isEmpty()) {
					...
					if(!visited.contains(next)) {
						...
						for (MapNode neighbor : neighbors) {
							if (!visited.contains(neighbor)) {
								if(next.compareTo(neighbor) == -1) {
									double distance = next.computeCost(neighbor);
									parentMap.put(neighbor, next);
									toExplore.add((new MapNode(neighbor, distance)));
								}
							}
						}
					}
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		for(GeographicPoint location: pointNodeMap.keySet()) {
			pointNodeMap.get(location).setCost(Double.POSITIVE_INFINITY);
			pointNodeMap.get(location).setPredictedCost(Double.POSITIVE_INFINITY);;
		}
		toExplore.add((new MapNode(startNode, 0.0, 0.0)));
		MapNode next = null;
		...
		while (!toExplore.isEmpty()) {
			...
			if(!visited.contains(next)) {
				...
				for (MapNode neighbor : neighbors) {
					if (!visited.contains(neighbor)) {
						if(next.compareToPredict(neighbor) == -1) {
							double distance = next.computeCost(neighbor);
							double predictedDistance = next.computePredictedCost(neighbor, distance);
							parentMap.put(neighbor, next);
							toExplore.add((new MapNode(neighbor,distance, predictedDistance)));
						}
					}
				}
			}
